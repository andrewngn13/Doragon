...
public class DamageHandlerTest
{
    ...
    private DamageRequest nanSource, sanSource;
    ...
    public void SetUp()
    {
        ...
        sanSource = new DamageRequest(false, ActionRole.Auxiliary, DamageType.Bash, ManaType.Blue, TargettingType.Single, new int[4], 1f, 0, rearguard, new Targets { PrimaryTarget = vanguard });
    }
    ...
    public void TestCorrectTargets()
    {
        ...
        // Nan to Nan
        damageHandler.PushDamageRequest(nanSource);
        Assert.IsTrue(damageHandler.GetDamageRequests().Where(r => r.source.Name == "Nan" && r.target.PrimaryTarget.Name == "Nan").Count() == 1);
        // Nan to San
        nanSource.target = new Targets { PrimaryTarget = rearguard };
        damageHandler.PushDamageRequest(nanSource);
        Assert.IsTrue(damageHandler.GetDamageRequests().Where(r => r.source.Name == "Nan" && r.target.PrimaryTarget.Name == "San").Count() == 1);
        // Nan to Ban
        nanSource.target = new Targets { PrimaryTarget = enemyFrontliner };
        damageHandler.PushDamageRequest(nanSource);
        Assert.IsTrue(damageHandler.GetDamageRequests().Where(r => r.source.Name == "Nan" && r.target.PrimaryTarget.Name == "Ban").Count() == 1);
        // Nan to Dan
        nanSource.target = new Targets { PrimaryTarget = enemyBackliner };
        damageHandler.PushDamageRequest(nanSource);
        Assert.IsTrue(damageHandler.GetDamageRequests().Where(r => r.source.Name == "Nan" && r.target.PrimaryTarget.Name == "Dan").Count() == 1);
    }
}
